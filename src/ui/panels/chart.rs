    ...
    match &state.selected_stock {
        ...
        Some(code) => {
            ...
            let chart_element: Element<'static, Message> = if state.intraday_period.is_some() && !state.intraday_bars.is_empty() {
                ...
                CandlestickCanvas::new(aggregated, state.time_range, state.zoom_level, state.hovered_bar_index, state.pan_offset, state.drawing_lines.clone(), &state.active_indicators).into_element()
            } else {
                text("").into()
            };
