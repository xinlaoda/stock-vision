...
impl CandlestickCanvas {
    pub fn new(bars: Vec<DailyBar>) -> Self {
        ...
        Self { bars, cache: canvas::Cache::new(), scroll_offset: 0, visible_count: visible }
    }
...
impl canvas::Program<crate::app::Message> for CandlestickCanvas {
    type State = ();
...
impl CandlestickCanvas {
    ...
    fn draw_candlesticks(&self, frame: &mut canvas::Frame) {
        ...
        for (i, bar) in bars.iter().enumerate() {
            ...
            let map_price = |p: f64| -> f32 { bottom_y - ((p - min_price) / price_range * chart_height as f64) as f32 };
            ...
            frame.fill_rectangle(Point::new(cx - 1.0, high_y), Size::new(2.0, (low_y - high_y).max(1.0)), color);
            let body_top = open_y.min(close_y);
            frame.fill_rectangle(Point::new(x, body_top), Size::new(bar_width, (open_y - close_y).abs().max(1.0)), color);
        }
